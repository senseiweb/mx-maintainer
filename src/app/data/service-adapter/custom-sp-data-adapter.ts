...
export class SpDataserviceAdapter {
    ...
    constructor() {
        ...
        ] = this.serverTypeNameToClient;
        this.jsonResultsAdapter[
            'clientTypeNameToServer'
        ] = this.clientTypeNameToServer;
        this.jsonResultsAdapter['typeMap'] = {
    ...
    private catchNoConnectionError(error): void {
        if (error.status === 0 && error.message == null) {
            error.message =
                'HTTP response status 0 and no message.  ' +
                'Likely did not or could not reach server. Is the server running?';
        }
    }

    private changeRequestSucceeded(
        savePackage: ISaveChngePackage,
        response: any,
        index: number
    ): Entity {
        ...
        switch (state) {
            case EntityState.Added:
                if (!type.defaultResourceName) {
                    throw new Error(
                        `Missing defaultResourceName for type ${type.name}`
                    );
                }
                ...
                rawEntity = helper.unwrapInstance(
                    entity,
                    this.transformSaveValue
                );

                rawEntity.__metadata = {
                    type: this.clientTypeNameToServer(type.shortName)
                };
                ...
                request = {
                    requestUri:
                        entityManager.dataService.serviceName +
            ...
            case EntityState.Deleted:
                ...
                if (!metadata) {
                    throw new Error(
                        'Missing the extra metadata for an update/delete entity'
                    );
                }
            ...
            default:
                throw new Error(
                    `Cannot save an entity whose EntityState is ${state.getName()}`
                );
        }
    ...
    executeQuery(mappingContext: MappingContext): Promise<QueryResult> {
        ...
        const entityType = this._getEntityTypeFromMappingContext(
            mappingContext
        );
        // @ts-ignore
        ...
        return new Promise((resolve, reject) => {
            httpOptions.success = response => {
                ...
                const inlineCount = data.__count
                    ? parseInt(data.__count, 10)
                    : undefined;
            ...
            httpOptions.error = response => {
                const error = this.createErrorFromResponse(
                    response,
    ...
    private _getEntityTypeFromMappingContext(mc: MappingContext): EntityType {
        ...
        if (!entityType) {
            ...
            if (etName) {
                entityType = mc.metadataStore.getEntityType(
                    etName
                ) as EntityType;
            }
        }
    ...
    saveChanges(
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const saveResult = this.reviewSaveResult(
                saveContext,
                savePackage,
                savePackage.requests
            );
    ...
    private reviewSaveResult(
        saveContext: SaveContext,
        savedPackage: ISaveChngePackage,
        requests: any[]
    ): ISaveChngePackage | any {
        if (!savedPackage.saveResult.entitiesWithErrors.length) {
            return savedPackage;
        }
        ...
        return savePackage.requests.map(async (request, index) => {
            ...
            const reqPromise = new Promise(async (resolve, reject) => {
                try {
                    ...
                    Promise.reject(
                        'Program error: failed while processing successful save response'
                    );
                }
            });
            return reqPromise;
    ...
    private tryRequestFailed(
        ...
        if (
            ...
            savePackage.originalEntities[
                index
            ].entityAspect.entityState.isDeleted()
        ) {
            ...
            savePackage.saveResult.entitiesWithErrors.push({
                entity: errorEntity,
                error: this.createErrorFromResponse(
                    response,
                    request.requestUri
                )
            });
    ...
    private tryRequestSucceeded(
        ...
        try {
            ...
            if (!serverResponseCode || serverResponseCode >= 400) {
                ...
                const savedEntity = this.changeRequestSucceeded(
                    savePackage,
                    response,
                    index
                );
    ...
    visitNode(
        ...
        const ignoreNode =
            node.__deferred != null || propertyName === '__metadata';
        ...
        if (!ignoreNode) {
            ...
            if (!entityType) {
                entityType = mappingContext.metadataStore.getEntityType(
                    typeName,
                    true
                ) as EntityType;
            ...
            if (
                entityType._mappedPropertiesCount -
                    entityType.navigationProperties.length <=
