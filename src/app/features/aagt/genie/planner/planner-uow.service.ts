...
import { TriggerActionRepoService, TriggerRepoService, ActionItem, ActionItemRepo, Trigger, TriggerAction, AssetTriggerAction, AagtDataModule } from '../../data';
import { Asset, AssetRepoService, GenAssetRepoService, Generation, GenerationAsset, GenerationRepoService } from '../../data';
...
export class PlannerUowService implements Resolve<any> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Promise<any> {
        ...
        return new Promise(async (resolve, reject) => {
            const dataNeeded = [this.fetchAllActionItems(), this.fetchISOoptions(), this.fetchAllAssets()];

            if (id !== 'new') {
                dataNeeded.concat([this.fetchGenerationAssets(), this.fetchGenerationTriggers()]);
            }

            try {
                const value = await Promise.all(dataNeeded).then(() => {
