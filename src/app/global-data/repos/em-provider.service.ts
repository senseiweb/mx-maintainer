...
import { config, AutoGeneratedKeyType, DataService, EntityManager, EntityType, MetadataStore, NamingConvention } from 'breeze-client';
...
export class EmProviderService {
    ...
    protected init(): void {
        const dataAdapter = config.initializeAdapterInstance('dataService', this.spDataAdapater.name, true);
        ...
        appEntities.forEach(entity => {
            const e = new entity();
            e.entityDefinition.createNamingDictionary(clientToServerNameDictionary, '');
        });
        ...
        appEntities.forEach(entity => {
            const e = new entity();
            const type = this.metadataHelper.addTypeToStore(metadataStore, e.entityDefinition as any) as EntityType;
            metadataStore.registerEntityTypeCtor(type.shortName, e.metadataFor, e.initializer);
    ...
    private fetchRequestDigest(): void {
        ...
        this.http.post(`${this.appCfg.apiAddress('')}contextinfo`, {}, { headers }).subscribe(response => {
