...
import { config, AutoGeneratedKeyType, DataType, Validator, MetadataStore, DataService, DataProperty, EntityType, ComplexType } from 'breeze-client';
import { EntityDefinition } from '../models/custom-entity-def';
...
export class CustomMetadataHelperService {
     ...
     */
    addTypeToStore(store: MetadataStore, typeDef: EntityDefinition<any>): EntityType | ComplexType {
        typeDef
            .checkForName()
        ...
        const type = typeDef.isComplexType ? new ComplexType(typeDef as any) : new EntityType(typeDef as any);
    ...
    addValidator(prop: DataProperty, validator: Validator): void {
        ...
        const found = validators.filter(function(val) {
            return val.name == valName;
        });
        if (!found.length) {
            // this validator has not already been specified
