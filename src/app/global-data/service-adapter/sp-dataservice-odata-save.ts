...
export class SpDataServiceOdataSave {
    constructor(
        ...
        const batchHeaders = this.utils.getRequestDigestHeaders(
            defaultBatchHeaders
        );
    ...
    private prepareSaveBundles(): void {
        ...
        this.saveBundle.entities.forEach((entity: SpEntityBase, index) => {
            ...
            switch (state) {
                ...
                case EntityState.Deleted:
                    request = this.processDeleteChangeSet(
                        entity as SpEntityBase,
                        contentId
                    );
                    break;
                default:
                    throw new Error(
                        `Cannot save an entity whose EntityState is ${
                            state.name
                        }`
                    );
            }
    ...
    private processAddChangeSet(
        ...
        const url =
            this.saveContext.entityManager.dataService.odataServiceEndpoint +
            et.defaultResourceName;
        ...
        return this.odataService.createRequest(
            'POST',
            url,
            payload,
            contentId.toString(),
    ...
    private processDeleteChangeSet(
        entity: SpEntityBase,
        contentId: number
    ): OData3Request {
    ...
    private processUpdateChangeSet(
        entity: Entity,
        contentId: number
    ): OData3Request {
        const em = this.saveContext.entityManager;
    ...
    private prepareSaveResult(responses: OData3Response[]): SaveResult {
        ...
        responses.forEach(response => {
            if (!response.Success) {
                const original = this.saveContext.originalEntities[
                    response.TempKeyIndex
                ];
            ...
            if (tempKey) {
                ...
                if (tmpEt.autoGeneratedKeyType !== AutoGeneratedKeyType.None) {
                    ...
                    saveResult.keyMappings.push(keyMapping);
                }
            }
