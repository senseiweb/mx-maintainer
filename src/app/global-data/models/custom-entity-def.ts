import { DataProperty, AutoGeneratedKeyType, NavigationProperty, DataType, EntityType } from 'breeze-client';

import * as pluralize from 'pluralize';
import { DataMembers, SpEntityDef, NavMembers, SpDataDef } from '@ctypes/breeze-type-customization';

interface INamingDictionary {
    [index: string]: { [index: string]: string };
}

export class EntityDefinition<T> implements SpEntityDef<T> {
    ...
    private removeAttribute(prop: Object, oldName: string, newName: string): EntityDefinition<any> {
    ...
    createNamingDictionary(dict: INamingDictionary, namespace: string): INamingDictionary {
        const propKeys = Object.keys(this.dataProperties);
        propKeys.forEach(propKey => {
            const dataProp = this.dataProperties[propKey];
            const dpKeys = Object.keys(dataProp);
            dpKeys.forEach(dpKey => {
                /**
                 * creates a naming dictionary to resolve the problem of sharepoint names maintain their
                 * original names, even after being changed on the site.
                ...
                if (dpKey === 'spInternalName' && dataProp.spInternalName) {
                    ...
                    if (dict[dictKey]) {
                        ...
                        const newKeyProp = { [dictKey]: dictProp };
                        Object.assign(dict, newKeyProp);
                    }
                }
            });
        });
    ...
    checkForName(): EntityDefinition<any> {
        if (this.name) {
    ...
    hasAKeyType(defaultKeyGenType: AutoGeneratedKeyType): EntityDefinition<any> {
    ...
    inferDefaultResourceName(): EntityDefinition<any> {
        if (this.isComplexType) {
            return this;
        }
    ...
    findEntityKey(): EntityDefinition<any> {
        if (this.isComplexType) {
            return this;
        }
    ...
    setNamespace(defaultNamespace: string): EntityDefinition<any> {
        this.namespace = this.namespace || defaultNamespace;
        return this;
    }

    replaceDataPropAliases(): EntityDefinition<any> {
        ...
        for (const oKey of objKeys) {
            const prop = this.dataProperties[oKey] as SpDataDef;
            ...
            for (const key of propKeys) {
                ...
                if (key === 'spInternalName' && prop.spInternalName) {
                    delete prop.spInternalName;
                }
            }
     ...
     */
    replaceNavPropAliases(): EntityDefinition<any> {
        ...
        for (const nKey of objKeys) {
            const prop = (this.navigationProperties[nKey] = this.normalizeNavProp(nKey, this.navigationProperties[nKey]) as any);
            ...
            if (prop.foreignKeyNames === undefined && prop.isScalar !== false && prop.invForeignKeyNames === undefined) {
                ...
                const fk = Object.keys(this.dataProperties).filter(function(k) {
                    k = k.toLowerCase();
                    return k === candidate1 || k === candidate2;
                })[0];
                if (fk) {
                    prop.foreignKeyNames = [fk];
                }
            }

            if (prop.associationName === undefined) {
                const isParent = prop.isScalar === false || prop.invForeignKeyNames || prop.foreignKeyNames === undefined;
                // association name is 'ChildType_ParentType'
                prop.associationName = (isParent ? propTypeName : this.typeName) + '_' + (isParent ? this.typeName : propTypeName);
            }
