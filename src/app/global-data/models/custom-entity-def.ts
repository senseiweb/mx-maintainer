...
export class EntityDefinition<T> implements SpEntityDef<T> {
    ...
    private removeAttribute(
        ...
        if (prop[newName] !== undefined) {
            throw new Error(
                `renameAttrib error; new name: ${newName} is already defined on object`
            );
        }
    ...
    createNamingDictionary(
        dict: INamingDictionary,
        namespace: string
    ): INamingDictionary {
    ...
    hasAKeyType(
        defaultKeyGenType: AutoGeneratedKeyType
    ): EntityDefinition<any> {
        if (this.isComplexType) {
            return this;
        }
        this.autoGeneratedKeyType =
            this.autoGeneratedKeyType || defaultKeyGenType;
        return this;
    }
    ...
    replaceDataPropAliases(): EntityDefinition<any> {
        ...
        for (const oKey of objKeys) {
            ...
            for (const key of propKeys) {
                ...
                if (keyLc === 'complex' || keyLc === 'complextype') {
                    ...
                    (prop.dataType === undefined ||
                        prop.dataType === DataType.String)
                ) {
                    this.removeAttribute(prop, key, 'maxLength');
                } else if (
                    keyLc.indexOf('null') > -1 &&
                    key !== 'isNullable' &&
            ...
            if (
                prop.complexTypeName &&
                prop.complexTypeName.indexOf(':#') === -1
            ) {
    ...
    replaceNavPropAliases(): EntityDefinition<any> {
        ...
        for (const nKey of objKeys) {
            const prop = (this.navigationProperties[
                nKey
            ] = this.normalizeNavProp(
                nKey,
            ...
            if (prop.associationName === undefined) {
                ...
                prop.associationName =
                    (isParent ? propTypeName : this.typeName) +
                    '_' +
